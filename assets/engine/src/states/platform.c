...
#define PLATFORM_CAMERA_DEADZONE_Y 4
#endif
...
void platform_update() BANKED {
    ...
    if (on_ladder) {
		...
		// Step Y - Coyote Time Variable Added
        grounded = FALSE;    
        ...
        if (pl_vel_y > 0) {
            ...
            while (tile_start != tile_end) {
                if (tile_at(tile_start, tile_y) & COLLISION_TOP) {
                    ...
                    grounded = TRUE;
                    pl_vel_y = 0;
					ct_val = 6; // Sets the Coyote Time to x frame window; default 6
                    break;
                }
                tile_start++;
            }
    ...
    // Platformer Jump Code
	// Coyote Time Jump
	if (INPUT_PRESSED(INPUT_PLATFORM_JUMP) && !grounded && ct_val != 0) {
		jb_val = 0;
		ct_val = 0;
	    pl_vel_y = -plat_jump_vel;
        grounded = FALSE;
	} else {	
		// Standard Jump/Jump Buffered Jump
